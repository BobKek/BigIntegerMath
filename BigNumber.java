...
public class BigNumber {
	...
	public int div(BigNumber b) {
		int answer = 1;
		BigNumber tmp = this;
		
		while(tmp.abs().CompareTo(b.abs()) == 1) {
			tmp = tmp.abs().Sub(b);
			answer++;
		}
		if(tmp.abs().CompareTo(b.abs()) == -1) {
			answer--;
		}
		return answer;
	}
		...
		while(true) {
			...
			switch(k[1]) {
				...
				case "+" : System.out.println(b1.Add(b2));
							break;
				case "-" : System.out.println(b1.Add(b2.neg()));
							break;
				case "^" : System.out.println(b1.pow(Integer.parseInt(k[2])));
							break;
				case "/" : System.out.println(b1.div(b2));
							break;
				default:
